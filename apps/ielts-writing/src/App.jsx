        ...
        body: JSON.stringify({
          ...
          generationConfig: { 
            ...
            responseSchema: {
              ...
              required: ["bandScore", "taskAchievement", "coherenceCohesion", "lexicalResource", "grammarAccuracy", "improvementTips", "modelHighlights", "diagnostic_tags"]
            }
          }
        })
      });
...
export default function App() {
  const [currentPrompt, setCurrentPrompt] = useState(() => PROMPT_BANK[Math.floor(Math.random() * PROMPT_BANK.length)]);
  ...
  const submitWriting = async () => {
    ...
    const systemPrompt = `You are a helpful IELTS Writing Examiner and expert EAP evaluator for beginner and intermediate students.
    Mark this Academic Task 1 response accurately, but explain your feedback using very simple, clear language. 
    Prompt: ${currentPrompt.instruction}. 
    Data Points: ${JSON.stringify(currentPrompt.data)}. 
    ...
    CRITICAL INSTRUCTION FOR DIAGNOSTIC TAGS:
    In addition to your written feedback, you MUST analyze the text for the following exact 20 error tags and return them in the "diagnostic_tags" array if the student struggled with them. ONLY use tags from this exact list. Do not make up your own tags.
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900">
      ...
      <main className="flex-1 overflow-hidden">
        {view === "bank" ? (
          <PromptList onSelect={startPractice} />
        ) : view === "feedback" ? (
          <FeedbackView 
            feedback={feedback} 
            originalText={text} 
            ...
            onNextRandom={() => startPractice(PROMPT_BANK[Math.floor(Math.random() * PROMPT_BANK.length)])}
          />
        ) : (
          <div className="h-full flex flex-col lg:flex-row">
            ...
            <div className="w-full lg:w-1/2 p-6 overflow-y-auto border-r bg-white scrollbar-hide">
              <div className="max-w-xl mx-auto">
                ...
                <div className="bg-white border shadow-sm rounded-2xl overflow-hidden mb-8 min-h-[350px] flex flex-col">
                  ...
                  <div className="flex-1 flex items-center justify-center bg-white p-4"><VisualRenderer prompt={currentPrompt} /></div>
                </div>
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 text-xs text-amber-900 flex gap-3"><AlertTriangle size={18} className="shrink-0 text-amber-500" /><p><b>Note:</b> You should write at least 150 words. Focus on reporting the main features and making comparisons.</p></div>
              </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex flex-col bg-slate-50">
              <div className="flex-1 p-6 flex flex-col">
                <div className="flex justify-between items-center mb-2 px-2"><span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Your Response</span><span className={`text-[10px] font-black uppercase ${getWordCount(text) < 150 ? 'text-amber-500' : 'text-green-500'}`}>{getWordCount(text)} words</span></div>
                <textarea className="flex-1 w-full p-8 rounded-2xl border border-slate-200 shadow-sm focus:ring-4 focus:ring-slate-200 outline-none text-lg leading-relaxed resize-none font-serif placeholder:text-slate-300 transition-all" placeholder="The chart illustrates..." value={text} onChange={handleTextChange} disabled={isLoading} />
              </div>
...
function PromptList({ onSelect }) {
  return (
    <div className="max-w-6xl mx-auto p-10 animate-in fade-in slide-in-from-bottom-2">
      <div className="mb-10 text-center"><h2 className="text-4xl font-black text-slate-900 mb-2 tracking-tight">Practice Bank</h2><p className="text-slate-500 font-medium">Select a data visualization task to begin your writing simulation.</p></div>
...
function FeedbackView({ feedback, originalText, saveMessage, onReset, onNextRandom }) {
    ...
    <div className="max-w-4xl mx-auto p-10 animate-in zoom-in-95 duration-500 relative">
      ...
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-100">
        <div className="bg-slate-900 p-12 text-center text-white relative overflow-hidden">
          <div className="inline-block px-4 py-1 bg-white/10 rounded-full text-[10px] font-black tracking-widest mb-6 border border-white/20 uppercase">Analytical report</div>
        ...
        <div className="p-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
            <div className="space-y-8">
              ...
              <Criterion label="Task Achievement" text={feedback.taskAchievement} />
              ...
              <Criterion label="Grammar Accuracy" text={feedback.grammarAccuracy} />
            </div>
          ...
          <div className="mb-12"><h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Final Submission</h3><div className="bg-slate-50 p-8 rounded-2xl font-serif text-slate-700 leading-loose italic border border-slate-100 shadow-inner whitespace-pre-wrap">{originalText}</div></div>
